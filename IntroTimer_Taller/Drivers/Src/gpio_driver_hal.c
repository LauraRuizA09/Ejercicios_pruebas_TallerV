...
void gpio_TooglePin(GPIO_Handler_t *pPinHandler){
	...
	uint8_t changeValue = gpio_ReadPin(pPinHandler);

	if (changeValue){

		gpio_WritePin(pPinHandler, 0);
	}
	else{

		gpio_WritePin(pPinHandler, 1);
	}
...
uint32_t gpio_ReadPin(GPIO_Handler_t *pPinHandler){
	...
	//poner de 8 bits por que no es necesario de 32
	//por que el retorno solo es 1 o 0
	...
	//lo estamos moviendo el registro hacia la izquierda no concuerda la posicion

	pinValue = (pPinHandler->pGPIOx->IDR >> pPinHandler->pinConfig.GPIO_PinNumber);

	//Como shiteamos a la derecha se conservaron los valores y para la mascara debemos borrarlos
	//con un and y un uno
