...
typedef struct{
	...
	uint8_t			TIMx_mode;				//Up or down (ver los cambios del flanco, contar)
	uint16_t		TIMx_Prescaler;			//Prescaler...
	uint32_t		TIMx_Period;			//Valor en ms del perido del timer
...
#define IS_TIMER_PERIOD(VALUE)			(((uint32_t)PERIOD) > 1)

void timer_Config(Timer_Handler_t *pTimerHandler);
void timer_SetState(Timer_Handler_t *pTimerHandler, uint8_t newState);

/*
 * ESta informacion debe ser sobre escrita e el main para que el sistema fucnione
 */
